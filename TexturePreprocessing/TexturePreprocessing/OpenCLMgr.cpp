...
OpenCLMgr::~OpenCLMgr()
{
	...
	//Release kernels
	if (xBlendKernel) status = clReleaseKernel(xBlendKernel);
    if (yBlendKernel) status = clReleaseKernel(yBlendKernel);
    if (xyBlendKernel) status = clReleaseKernel(xyBlendKernel);
    if (testMosaicKernel) status = clReleaseKernel(testMosaicKernel);
...
cl_int OpenCLMgr::init()
{
	...
	const char *filename = "./TexturePreprocessing/image_proc.cl";
	string sourceStr;
	...
	xBlendKernel = clCreateKernel(program,"xblend", &status);
	CHECK_SUCCESS("Error: creating xBlendKernel")
    yBlendKernel = clCreateKernel(program,"yblend", &status);
	CHECK_SUCCESS("Error: creating yBlendKernel")
    xyBlendKernel = clCreateKernel(program,"xyblend", &status);
    CHECK_SUCCESS("Error: creating xyBlendKernel")
    testMosaicKernel= clCreateKernel(program,"testMosaic", &status);
    CHECK_SUCCESS("Error: creating testMosaicKernel")
