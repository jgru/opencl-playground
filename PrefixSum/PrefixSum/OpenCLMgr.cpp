...
OpenCLMgr::~OpenCLMgr()
{
	...
	//Release kernels
	if (prefixSumKernel) status = clReleaseKernel(prefixSumKernel);
    if (mergeKernel) status = clReleaseKernel(mergeKernel);
...
/* convert the prefixSumKernel file into a string */
...
cl_int OpenCLMgr::init()
{
	...
	const char *filename = "./Sum_Kernel.cl";
	string sourceStr;
	...
	prefixSumKernel = clCreateKernel(program,"PrefixSumKernel", &status);
	CHECK_SUCCESS("Error: creating SumKernel kernel")
    mergeKernel = clCreateKernel(program,"MergeKernel", &status);
	CHECK_SUCCESS("Error: creating mergeKernel kernel")
